...
function HousePanel() {
  ...
  const fetchProperties = async () => {
    ...
    setLoadError(null)
    try {
      const res = await fetch(`${API}?type=house`)
      if (!res.ok) throw new Error(`Request failed with status ${res.status}`)
      const data = await res.json()
      setProperties(normalizeList(data))
    } catch (e) {
      console.error(e)
      setLoadError('სახლების ჩატვირთვა ვერ მოხერხდა')
      setProperties([])
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div>
      <div className="panel-header">
        <div className="panel-title">სახლები <span>{list.length} განცხადება</span></div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>სახლები არ არის დამატებული</p>
        </div>
      ) : (
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal">
            ...
            <div className="modal-header">
              <div className="modal-title">{editing ? 'სახლის რედაქტირება' : 'სახლის დამატება'}</div>
            ...
            <div className="modal-body">
              ...
              <div className="form-grid" style={{marginBottom: '24px'}}>
                <div className="form-field">
                  ...
                  <input className="form-input" value={form.title.geo} onChange={e => bi('title','geo',e.target.value)} placeholder="სახლი ვაკეში" />
                </div>
                <div className="form-field">
                  <label className="form-label">English</label>
                  <input className="form-input" value={form.title.eng} onChange={e => bi('title','eng',e.target.value)} placeholder="House in Vake" />
                </div>
                <div className="form-field" />
              </div>
              ...
              <div className="form-grid" style={{marginBottom: '24px'}}>
                ...
                <div className="form-field">
                  <label className="form-label">საძინებელი</label>
                  <input className="form-input" type="number" value={form.bedrooms} onChange={e => fld('bedrooms', e.target.value)} placeholder="2" />
                </div>
                <div className="form-field">
              ...
              <div className="form-grid" style={{marginBottom: '24px'}}>
                <div className="form-field" style={{gridColumn: 'span 1'}}>
                  <label className="form-label">ქართული</label>
                  <textarea className="form-textarea" value={form.description.geo} onChange={e => bi('description','geo',e.target.value)} placeholder="სახლის აღწერა..." />
                </div>
                <div className="form-field" style={{gridColumn: 'span 1'}}>
                  <label className="form-label">English</label>
